...
class Bot extends Client {
  /** @param {import('../resources/Global/clientConfig.mjs').clientConfig} config */
  constructor(config) {
    super({
      allowedMentions: {
  ...
  /**
   *
   * @param {import("../resources/Global/clientConfig.mjs").clientConfig} query
   * @returns
   */
  ...
  async getCommands(type) {
    ...
    if (type === "Slash") {
      try {
        const dirs = await fs.readdir("./cmd/Slash");

        for (const dir of dirs) {
          const files = await fs.readdir(`./cmd/Slash/${dir}/`);
      ...
      const dirs = await fs.readdir("./cmd/Prefix");

      for (const dir of dirs) {
        const files = (await fs.readdir(`./cmd/Prefix/${dir}/`)).filter(
